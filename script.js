...
function showPwaInstallNotice(force = false) {
    if (isStandalonePwa() || (!deferredPwaInstallPrompt && !isIosPwaDevice())) return;
...
function updatePwaInstallUi(forcedState = '') {
    ...
    const canOfferInstall = nativeInstallAvailable || manualIosInstall;

    if (footerButton) {
        footerButton.hidden = !canOfferInstall;
    ...
    if (footerStatus) {
        footerStatus.textContent = nativeInstallAvailable
            ...
            : manualIosInstall
                ...
                : 'Installation non disponible dans ce navigateur';
    }
    ...
    if (noticeText) {
        noticeText.textContent = nativeInstallAvailable
            ...
            : manualIosInstall
                ...
                : 'L’installation n’est pas proposée par ce navigateur pour le moment.';
    }

    if (!canOfferInstall || forcedState === 'unsupported') hidePwaInstallNotice(false);
}
