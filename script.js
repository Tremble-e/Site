...
const VALID_SECTIONS = ['about', 'services', 'projects', 'courses', 'planning', 'forum', 'library', 'notifications'];
...
/* Configuration Supabase. La clé publishable peut rester côté client, jamais la service_role. */
...
/* Bibliothèque personnelle et mode hors connexion */
...
/* État en mémoire du contenu public chargé depuis Supabase. */
let myProjects = [];
let generalInfo = [];
let myCourses = [];
let globalResources = [];
...
async function loadPublicHomeStatus() {
    ...
    try {
        ...
        let lastSyncedAt = row?.last_synced_at || null;
...
/* Effets de survol et progression de page. */
...
function updatePwaInstallUi(forcedState = '') {
    ...
    // Garde le bouton disponible quand l’installation native n’est pas proposée.
    if (footerButton) {
...
async function installSitePwa() {
    ...
    // Repli pour les navigateurs sans beforeinstallprompt.
    showPwaInstallNotice(true);
...
const themePresets = {
    ...
    amber: { color: '#eab308', hover: '#ca8a04' },
    green: { color: '#22c55e', hover: '#16a34a' },
    teal: { color: '#14b8a6', hover: '#0f766e' },
...
function buildAccentPalette(color, hover = '') {
    const primary = normalizeHexColor(color);
    const rgb = hexToRgb(primary);
    const hsl = rgbToHsl(rgb);
    const secondary = hslColor({ h: hsl.h + 34, s: clamp(hsl.s + 4, 55, 92), l: clamp(hsl.l + 5, 48, 66) });
    const tertiary = hslColor({ h: hsl.h + 72, s: clamp(hsl.s + 2, 55, 90), l: clamp(hsl.l + 7, 50, 68) });
    const light = hslColor({ h: hsl.h, s: clamp(hsl.s * .72, 40, 85), l: 76 });
    const pale = hslColor({ h: hsl.h, s: clamp(hsl.s * .55, 32, 78), l: 90 });
    const secondaryHsl = rgbToHsl(hexToRgb(secondary));
    const tertiaryHsl = rgbToHsl(hexToRgb(tertiary));

    return {
        primary,
        hover: hover || hslColor({ h: hsl.h, s: hsl.s, l: clamp(hsl.l - 9, 30, 58) }),
        rgb,
        secondary,
        ...
        light,
        pale,
        secondaryLight: hslColor({ h: secondaryHsl.h, s: secondaryHsl.s * .7, l: 78 }),
        secondaryPale: hslColor({ h: secondaryHsl.h, s: secondaryHsl.s * .55, l: 90 }),
        tertiaryLight: hslColor({ h: tertiaryHsl.h, s: tertiaryHsl.s * .7, l: 78 }),
        tertiaryPale: hslColor({ h: tertiaryHsl.h, s: tertiaryHsl.s * .55, l: 90 })
    };
}
...
function buildFallbackAvatarDataUri(accentColor) {
    const accent = normalizeHexColor(accentColor);
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" rx="128" fill="#111b30"/><circle cx="128" cy="96" r="44" fill="${accent}" fill-opacity=".22"/><path d="M53 221c10-45 36-68 75-68s65 23 75 68" fill="${accent}" fill-opacity=".22"/><text x="128" y="140" text-anchor="middle" font-size="58" font-family="Arial,sans-serif" font-weight="700" fill="#e2e8f0">LC</text></svg>`;
    return `data:image/svg+xml,${encodeURIComponent(svg)}`;
}

function updateThemeFallbackAvatars(accentColor) {
    const fallbackSrc = buildFallbackAvatarDataUri(accentColor);
    document.querySelectorAll('[data-theme-fallback-avatar]').forEach(img => {
        const source = img.getAttribute('src') || '';
        if (source.startsWith('data:image/svg+xml')) img.src = fallbackSrc;
    });
}

function applyTheme(name, persist = true, customColor = '') {
    const isCustom = name === 'custom';
    const savedCustom = localStorage.getItem('portfolio-custom-accent') || '#3b82f6';
    const themeName = isCustom ? 'custom' : (themePresets[name] ? name : 'blue');
    const base = isCustom
        ? { color: normalizeHexColor(customColor || savedCustom), hover: '' }
        : themePresets[themeName];
    const palette = buildAccentPalette(base.color, base.hover);
    const root = document.documentElement;

    root.style.setProperty('--accent-color', palette.primary);
    root.style.setProperty('--accent-hover', palette.hover);
    root.style.setProperty('--accent-rgb', rgbValue(palette.rgb));
    root.style.setProperty('--accent-soft', `rgba(${rgbValue(palette.rgb)}, 0.14)`);
    root.style.setProperty('--accent-secondary', palette.secondary);
    root.style.setProperty('--accent-secondary-rgb', rgbValue(palette.secondaryRgb));
    root.style.setProperty('--accent-tertiary', palette.tertiary);
    root.style.setProperty('--accent-tertiary-rgb', rgbValue(palette.tertiaryRgb));
    root.style.setProperty('--accent-light', palette.light);
    root.style.setProperty('--accent-pale', palette.pale);
    root.style.setProperty('--accent-secondary-light', palette.secondaryLight);
    root.style.setProperty('--accent-secondary-pale', palette.secondaryPale);
    root.style.setProperty('--accent-tertiary-light', palette.tertiaryLight);
    root.style.setProperty('--accent-tertiary-pale', palette.tertiaryPale);
    ...
    if (customPicker) customPicker.value = palette.primary;

    updateThemeFallbackAvatars(palette.primary);
    const browserTheme = document.querySelector('meta[name="theme-color"]');
    if (browserTheme) {
        const baseHsl = rgbToHsl(palette.rgb);
        browserTheme.content = hslColor({ h: baseHsl.h, s: clamp(baseHsl.s * .55, 30, 70), l: 11 });
    }

    if (persist) {
        localStorage.setItem('portfolio-accent-theme', themeName);
        if (isCustom) localStorage.setItem('portfolio-custom-accent', palette.primary);
    }
...
customThemeColor?.addEventListener('input', () => applyTheme('custom', true, customThemeColor.value));
customThemeColor?.addEventListener('change', () => applyTheme('custom', true, customThemeColor.value));
...
// API partagée avec les modules du forum et des comptes.
...
// Branche les événements de l’administration.
...
function setImageLightboxZoom(nextScale, clientX = null, clientY = null) {
    ...
    // Conserve le point visé pendant le zoom.
...
imageLightboxStage?.addEventListener('wheel', event => {
    // Les trackpads envoient souvent le pincement sous forme de Ctrl + molette.
    event.preventDefault();
    event.stopPropagation();
...
// Empêche Safari/iOS de zoomer la page derrière la visionneuse.
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const savedCustomColor = localStorage.getItem('portfolio-custom-accent') || '';
    applyTheme(savedTheme, false, savedCustomColor);
