...
async function registerSitePwa() {
    ...
    try {
        ...
        updatePwaInstallUi();
    } catch (error) { console.warn('Service Worker :', error); updatePwaInstallUi('unsupported'); }
}
...
function updatePwaInstallUi(forcedState = '') {
    ...
    if (isStandalonePwa()) {
        button.disabled = true;
        button.innerHTML = '<i class="fa-solid fa-circle-check"></i> Installée';
        status.textContent = 'Le site est déjà installé comme une application sur cet appareil.';
    } else if (deferredPwaInstallPrompt) {
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-download"></i> Installer';
        status.textContent = 'Installation disponible : lancement plein écran et accès plus rapide.';
    } else if (ios) {
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-share-nodes"></i> Comment installer';
        status.textContent = 'Sur iPhone/iPad : Partager → Ajouter à l’écran d’accueil.';
    } else if (forcedState === 'unsupported' || !('serviceWorker' in navigator)) {
        button.disabled = true;
        button.innerHTML = '<i class="fa-solid fa-ban"></i> Indisponible';
        status.textContent = 'Ce navigateur ne propose pas l’installation de l’application.';
    } else {
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-download"></i> Installer';
        status.textContent = 'Le navigateur proposera l’installation dès que les critères PWA seront validés.';
    }
}
...
async function installSitePwa() {
    ...
    } else showToast('Utilisez l’option « Installer l’application » de votre navigateur si elle est disponible.');
}
...
window.addEventListener('appinstalled', () => { deferredPwaInstallPrompt = null; updatePwaInstallUi(); showToast('Application installée.'); });
...
document.getElementById('pwa-install-btn')?.addEventListener('click', installSitePwa);
