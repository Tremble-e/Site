...
function startRotatingText() {
    ...
    const shell = element?.closest('.rotating-text-shell');
    if (!element) return;
    ...
    const syncShellWidth = () => {
        if (!shell) return;
        shell.style.width = `${measureTextWidth(element.textContent || '')}px`;
    };
    syncShellWidth();
    ...
    window.setInterval(() => {
        ...
        window.setTimeout(() => {
            index = nextIndex;
            element.textContent = nextText;
            if (shell) shell.style.width = `${measureTextWidth(nextText)}px`;
