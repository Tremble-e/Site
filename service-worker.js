...
const SHELL_CACHE = 'tremble-shell-v2.9.1';
const RUNTIME_CACHE = 'tremble-runtime-v2.9.1';
...
self.addEventListener('activate', event => {
    event.waitUntil((async () => {
        ...
        await Promise.all(keys
            .filter(key => key.startsWith('tremble-shell-') && key !== SHELL_CACHE)
...
self.addEventListener('fetch', event => {
    ...
    // Les documents enregistrés explicitement hors connexion ont priorité,
    // qu'ils viennent de Supabase, GitHub Pages ou d'une autre URL publique.
