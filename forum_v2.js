...
async function loadPrivateThreads() {
    ...
    list.innerHTML = data?.length ? data.map(t => {
        ...
        const detail = group ? `${Number(t.participant_count || 0)} membres · ${(t.last_message || 'Nouveau groupe').slice(0, 70)}` : (t.last_message || 'Nouvelle conversation').slice(0, 80);
        return `<button type="button" class="private-thread-item ${String(t.id) === String(forumV2State.currentPrivateThreadId) ? 'active' : ''}" data-private-thread="${v2Attr(t.id)}">
            ${avatar}
            <span><strong>${v2Esc(name)}</strong><small>${v2Esc(detail)}</small></span>
...
function renderPrivateHeadParticipants(members) {
    return `<span class="private-head-avatars">${members.slice(0, 4).map(m => privateAvatar(m, 'tiny')).join('')}${members.length > 4 ? `<span class="private-head-more">+${members.length - 4}</span>` : ''}</span>`;
}
...
async function openPrivateThread(threadId) {
    ...
    const subtitle = isGroup ? `${members.length} membre${members.length > 1 ? 's' : ''} · groupe privé` : 'Conversation privée';

    pane.innerHTML = `
        <div class="private-chat-head">
            <div class="private-chat-identity">${isGroup ? privateGroupAvatar() : privateAvatar(other || { username: title }, 'head')}<div><strong>${v2Esc(title)}</strong><small>${v2Esc(subtitle)}</small></div></div>
        ...
        <div id="private-message-list" class="private-message-list">${messages?.length ? messages.map(m => {
            ...
            return `<article class="private-message ${mine ? 'mine' : ''}"><div class="private-message-meta"><button type="button" class="private-message-author" data-private-member-profile="${v2Attr(author.user_id || '')}">${v2Esc(author.username)}</button><span>${fRelativeDate(m.created_at)}</span></div><div class="private-message-body">${m.deleted_at ? '<em>Message supprimé.</em>' : renderForumText(m.content)}</div></article>`;
...
function renderPrivateDirectResults() {
    ...
    host.innerHTML = rows.length ? rows.map(p => `
        <button type="button" class="private-direct-member" data-private-direct-user="${v2Attr(p.user_id)}">
            ${privateAvatar(p)}
            <span><strong>${v2Esc(p.username)}</strong><small>${p.role === 'admin' ? 'Admin' : p.role === 'moderator' ? 'Modérateur' : 'Membre'}</small></span>
            <i class="fa-solid fa-chevron-right"></i>
        </button>`).join('') : '<div class="admin-empty compact"><p>Aucun membre correspondant.</p></div>';
}
...
function renderPrivateGroupCreateMembers() {
    ...
    host.innerHTML = rows.length ? rows.map(p => {
        ...
        return `<label class="private-group-candidate ${checked ? 'selected' : ''}"><input type="checkbox" data-private-group-create-member="${v2Attr(p.user_id)}" ${checked ? 'checked' : ''}>${privateAvatar(p)}<span><strong>${v2Esc(p.username)}</strong><small>${p.role === 'admin' ? 'Admin' : p.role === 'moderator' ? 'Modérateur' : 'Membre'}</small></span><i class="fa-solid fa-check"></i></label>`;
...
function renderPrivateGroupMemberList() {
    ...
    const creator = String(summary.creator_id || '') === String(forumState.user?.id || '');
    ...
    host.innerHTML = members.map(member => {
        const isCreator = String(member.user_id) === String(summary.creator_id || '');
        const canRemove = Boolean(summary.is_group && creator && !isCreator);
        return `<div class="private-group-member-row"><button type="button" class="private-group-member-profile" data-private-member-profile="${v2Attr(member.user_id)}">${privateAvatar(member)}<span><strong>${v2Esc(member.username)}</strong><small>${isCreator ? 'Créateur du groupe' : (member.role === 'admin' ? 'Admin' : member.role === 'moderator' ? 'Modérateur' : 'Membre')}</small></span></button>${canRemove ? `<button type="button" class="private-group-remove-btn" data-private-group-remove="${v2Attr(member.user_id)}" data-tooltip="Retirer du groupe"><i class="fa-solid fa-user-minus"></i></button>` : ''}</div>`;
    }).join('');
}
...
function renderPrivateGroupAddResults() {
    ...
    host.innerHTML = rows.length ? rows.map(p => `<div class="private-group-add-row">${privateAvatar(p)}<span><strong>${v2Esc(p.username)}</strong><small>${p.role === 'admin' ? 'Admin' : p.role === 'moderator' ? 'Modérateur' : 'Membre'}</small></span><button type="button" class="private-group-add-btn" data-private-group-add="${v2Attr(p.user_id)}"><i class="fa-solid fa-user-plus"></i> Ajouter</button></div>`).join('') : '<div class="admin-empty compact"><p>Aucun autre membre.</p></div>';
}
...
async function openPrivateGroupMembers() {
    ...
    if (subtitle) subtitle.textContent = summary.is_group ? 'Tous les participants peuvent ajouter un membre. Seul le créateur peut en retirer.' : 'Ajouter quelqu’un transformera cette conversation en groupe. Vous en deviendrez le créateur.';
    try {
        ...
        if (leaveHelp && summary.is_group) leaveHelp.textContent = canRename
            ? 'En quittant le groupe, votre rôle de créateur sera automatiquement transmis à un autre membre.'
...
async function performAddPrivateMemberToCurrent(userId) {
    ...
    const subtitle = document.getElementById('private-group-members-subtitle');
    if (subtitle && refreshed?.is_group) subtitle.textContent = 'Tous les participants peuvent ajouter un membre. Seul le créateur peut en retirer.';
    const leaveZone = document.getElementById('private-group-leave-zone');
...
async function leavePrivateGroup() {
    ...
    const isCreator = String(summary.creator_id || '') === String(forumState.user.id);
    const accepted = await siteConfirm({
        ...
        detail: isCreator
            ? 'Vous êtes le créateur : le rôle sera automatiquement transmis au membre présent depuis le plus longtemps.'
    ...
    forumV2State.privateChannel = client.channel(`private-messages-${forumState.user.id}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'private_messages' }, async payload => {
            if (document.getElementById('privateMessagesModal')?.classList.contains('active')) {
                await loadPrivateThreads();
                if (String(payload.new?.thread_id || '') === String(forumV2State.currentPrivateThreadId || '')) await openPrivateThread(forumV2State.currentPrivateThreadId);
            }
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'private_thread_members' }, async () => {
            if (document.getElementById('privateMessagesModal')?.classList.contains('active')) await loadPrivateThreads();
        })
        .subscribe();
}
...
function bindForumV2Events() {
    ...
    ['forumReferenceModal', 'forumAttachmentModal', 'forumHistoryModal', 'privateMessagesModal', 'privateDirectCreateModal', 'privateGroupCreateModal', 'privateGroupMembersModal', 'privateAddChoiceModal', 'adminV2TopicModal', 'adminV2MessageModal'].forEach(id => {
    ...
    document.addEventListener('keydown', event => {
        ...
        ['forumReferenceModal', 'forumAttachmentModal', 'forumHistoryModal', 'privateMessagesModal', 'privateDirectCreateModal', 'privateGroupCreateModal', 'privateGroupMembersModal', 'privateAddChoiceModal', 'adminV2TopicModal', 'adminV2MessageModal'].forEach(closeModalById);
    });
}
