...
renderForumText = function forumV2RenderForumText(raw) {
    const tokens = [];
    const protectedText = String(raw || '').replace(/\[\[(?:reply:\d+\|[^\]]+|ref:(?:project|document|subject|info|topic|post|member):[^\]]+|media:(?:image|file):https?:\/\/[^\]]+)\]\]/gi, match => {
...
function replyToForumPostV2(postId) {
    ...
    if (!post || post.deleted_at) return v2Toast('Ce message n’est plus disponible.');
    if (forumState.currentTopic?.is_locked && !forumState.isModerator) return v2Toast('Cette discussion est verrouillée.');

    const textarea = document.getElementById('forum-reply-content');
    if (!textarea) return v2Toast('La zone de réponse n’est pas disponible.');
    const profile = forumState.profilesById.get(post.author_id);
    const username = profile?.username || 'Membre';
    const safeUser = String(username).replace(/[|\]]/g, '').slice(0, 30);
    const excerpt = v2PlainExcerpt(post.content).replace(/[|\]]/g, '') || 'Message du forum';
    const mention = post.author_id && post.author_id !== forumState.user.id ? `\n@${safeUser} ` : '\n';
    v2InsertAtCursor(textarea, `[[reply:${Number(post.id)}|${safeUser}|${excerpt}]]${mention}`);
