...
function renderPrivateGroupMemberList() {
    ...
    host.innerHTML = members.map(member => {
        ...
        return `<div class="private-group-member-row"><button type="button" class="private-group-member-profile" data-private-member-profile="${v2Attr(member.user_id)}">${privateAvatar(member)}<span><strong>${v2Esc(member.username)}</strong><small>${roleLabel}</small></span></button>${actions}</div>`;
    }).join('');
}
