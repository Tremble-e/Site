...
const forumV2State = {
    ...
    notificationCenterUnreadOnly: false
};
...
// Conserve l’échappement HTML lors de l’ajout des liens riches.
...
/* Références, mentions et pièces jointes */
...
renderAccountPopover = function forumV2RenderAccountPopover() {
    ...
    // Ces raccourcis existent déjà dans la navigation principale.
...
async function openPrivateThread(threadId) {
    ...
    // Préserve le brouillon et le focus pendant les mises à jour temps réel.
    if (draftState) {
...
async function setupPrivateRealtimeV2() {
    ...
    const refreshMembership = () => {
        // Regroupe les rafraîchissements quand plusieurs membres changent à la suite.
        clearTimeout(forumV2State.privateMembershipRefreshTimer);
    ...
    forumV2State.privateChannel = client.channel(`private-messages-${forumState.user.id}`)
        ...
        // On ignore UPDATE ici : last_read_at provoquerait sinon une boucle de rafraîchissement.
...
renderNotifications = function forumV2RenderNotifications(items) {
    ...
    pop.innerHTML = `<div class="notification-head"><strong>Notifications</strong>${unread ? '<button type="button" data-notifications-read-all>Tout marquer comme lu</button>' : ''}</div><div class="notification-list">${rows}</div><div class="notification-popover-footer"><button type="button" data-notification-center-open><i class="fa-solid fa-sliders"></i> Centre de notifications</button></div>`;
};


function notificationCenterCategory(item) {
    if (item?.type === 'site_info') return item.info_section === 'projects' ? 'projects' : 'studies';
    if (item?.private_thread_id || item?.type === 'private_message') return 'private';
    return 'forum';
}

function notificationCenterIcon(item) {
    const icons = {
        reaction: 'fa-regular fa-heart', mention: 'fa-solid fa-at', subscription: 'fa-regular fa-bell',
        private_message: 'fa-regular fa-paper-plane', badge: 'fa-solid fa-award', reply: 'fa-regular fa-message',
        system: 'fa-solid fa-circle-info', site_info: 'fa-solid fa-bullhorn'
    };
    return icons[item?.type] || 'fa-regular fa-bell';
}

function notificationCenterCategoryLabel(category) {
    return { forum: 'Forum', private: 'Messages privés', studies: 'Études', projects: 'Projets' }[category] || 'Notification';
}

function renderNotificationCenter() {
    const list = document.getElementById('notification-center-list');
    const totalEl = document.getElementById('notification-center-total-count');
    const unreadEl = document.getElementById('notification-center-unread-count');
    if (!list) return;

    const items = forumV2State.notificationCenterItems || [];
    const unreadCount = items.filter(item => !item.is_read).length;
    if (totalEl) totalEl.textContent = String(items.length);
    if (unreadEl) unreadEl.textContent = String(unreadCount);

    document.querySelectorAll('[data-notification-filter]').forEach(btn => btn.classList.toggle('active', btn.dataset.notificationFilter === forumV2State.notificationCenterFilter));
    const unreadToggle = document.getElementById('notification-center-unread-only');
    if (unreadToggle) unreadToggle.checked = forumV2State.notificationCenterUnreadOnly;

    if (!forumState.user) {
        list.innerHTML = '<div class="notification-center-empty"><i class="fa-regular fa-user"></i><h3>Connectez-vous</h3><p>Votre historique de notifications est lié à votre compte.</p><button type="button" class="primary-btn compact-btn" data-open-account><i class="fa-solid fa-right-to-bracket"></i> Se connecter</button></div>';
        return;
    }

    const filtered = items.filter(item => {
        const category = notificationCenterCategory(item);
        return (forumV2State.notificationCenterFilter === 'all' || category === forumV2State.notificationCenterFilter)
            && (!forumV2State.notificationCenterUnreadOnly || !item.is_read);
    });

    if (!filtered.length) {
        list.innerHTML = '<div class="notification-center-empty"><i class="fa-regular fa-bell-slash"></i><h3>Aucune notification</h3><p>Aucune activité ne correspond aux filtres sélectionnés.</p></div>';
        return;
    }

    list.innerHTML = filtered.map(item => {
        const category = notificationCenterCategory(item);
        return `<button type="button" class="notification-center-item ${item.is_read ? '' : 'unread'}" data-notification-id="${item.id}" data-notification-type="${v2Attr(item.type || '')}" data-notification-actor="${v2Attr(item.actor_id || '')}" data-notification-topic="${v2Attr(item.topic_id || '')}" data-notification-post="${v2Attr(item.post_id || '')}" data-notification-thread="${v2Attr(item.private_thread_id || '')}" data-notification-info="${v2Attr(item.info_id || '')}" data-notification-info-section="${v2Attr(item.info_section || '')}">
            <span class="notification-center-item-icon ${category}"><i class="${notificationCenterIcon(item)}"></i></span>
            <span class="notification-center-item-copy"><small>${notificationCenterCategoryLabel(category)} · ${fRelativeDate(item.created_at)}</small><strong>${v2Esc(item.message || 'Nouvelle notification')}</strong><span>${v2Esc(notificationDestinationLabel(item))}</span></span>
            ${item.is_read ? '' : '<span class="notification-center-unread-dot" aria-label="Non lue"></span>'}
            <i class="fa-solid fa-chevron-right" aria-hidden="true"></i>
        </button>`;
    }).join('');
}

async function loadNotificationCenterPreferences() {
    const forumToggle = document.getElementById('notification-pref-forum');
    const privateToggle = document.getElementById('notification-pref-private');
    const studyToggle = document.getElementById('notification-sub-studies');
    const projectToggle = document.getElementById('notification-sub-projects');
    if (!forumState.user) {
        [forumToggle, privateToggle, studyToggle, projectToggle].forEach(el => { if (el) el.disabled = true; });
        return;
    }
    [forumToggle, privateToggle, studyToggle, projectToggle].forEach(el => { if (el) el.disabled = false; });

    const client = v2Client();
    const [{ data: pref, error: prefError }, { data: subscriptions, error: subsError }] = await Promise.all([
        client.from('user_notification_preferences').select('forum_enabled,private_messages_enabled').eq('user_id', forumState.user.id).maybeSingle(),
        client.from('site_info_subscriptions').select('section').eq('user_id', forumState.user.id)
    ]);
    if (prefError) console.warn('Préférences de notifications :', prefError);
    if (subsError) console.warn('Abonnements informations :', subsError);
    if (forumToggle) forumToggle.checked = pref?.forum_enabled !== false;
    if (privateToggle) privateToggle.checked = pref?.private_messages_enabled !== false;
    const sections = new Set((subscriptions || []).map(row => row.section));
    if (studyToggle) studyToggle.checked = sections.has('studies');
    if (projectToggle) projectToggle.checked = sections.has('projects');
}

async function saveNotificationCenterPreference(field, enabled) {
    if (!forumState.user || !['forum_enabled', 'private_messages_enabled'].includes(field)) return;
    const client = v2Client();
    ...
    if (error) throw error;
    v2Toast('Préférence de notification enregistrée.');
}

async function loadNotificationCenter() {
    const list = document.getElementById('notification-center-list');
    if (!forumState.user) {
        forumV2State.notificationCenterItems = [];
        renderNotificationCenter();
        await loadNotificationCenterPreferences();
        return;
    }
    if (list) list.innerHTML = '<div class="notification-center-loading"><i class="fa-solid fa-circle-notch fa-spin"></i><span>Chargement de l’historique…</span></div>';
    const { data, error } = await v2Client().from('notifications')
        .select('*').eq('user_id', forumState.user.id).order('created_at', { ascending: false }).limit(250);
    if (error) {
        console.warn(error);
        if (list) list.innerHTML = '<div class="notification-center-empty"><i class="fa-solid fa-triangle-exclamation"></i><h3>Notifications indisponibles</h3><p>Impossible de charger l’historique pour le moment.</p></div>';
        return;
    }
    forumV2State.notificationCenterItems = data || [];
    renderNotificationCenter();
    await loadNotificationCenterPreferences();
}
...
async function markAllNotificationCenterRead() {
    if (!forumState.user) return;
    const { error } = await v2Client().from('notifications').update({ is_read: true }).eq('user_id', forumState.user.id).eq('is_read', false);
    if (error) return v2Toast(error.message || 'Impossible de mettre à jour les notifications.');
    await Promise.all([loadNotifications(), loadNotificationCenter()]);
}

window.openNotificationCenter = openNotificationCenter;
window.refreshNotificationCenter = () => {
    if (document.getElementById('notifications')?.classList.contains('active')) loadNotificationCenter().catch(console.warn);
};
...
openNotification = async function forumV2OpenNotification(button) {
    ...
    // Ouvre la rubrique liée et met l’annonce en évidence.
    if (type === 'site_info') {
    ...
    // Ouvre directement la conversation privée ciblée.
    ...
    // Cible le message exact pour les réponses, mentions et réactions.
    if (postId && !topicId) {
    ...
    // Les badges sont visibles sur le profil du membre connecté.
    if (type === 'badge' && forumState.user) {
        await openMemberProfile(forumState.user.id);
        return;
    }

    // Replie les notifications sans cible précise vers le forum.
    goToForum();
};
...
/* Administration des sujets et messages */
...
function bindForumV2Events() {
    ...
    document.addEventListener('click', async event => {
        ...
        if (target.closest('[data-notification-center-open]')) { await openNotificationCenter(); return; }
        if (target.closest('[data-notifications-center-read-all]')) { await markAllNotificationCenterRead(); return; }
    ...
    document.querySelectorAll('[data-notification-filter]').forEach(button => button.addEventListener('click', () => {
        forumV2State.notificationCenterFilter = button.dataset.notificationFilter || 'all';
        renderNotificationCenter();
    }));
    document.getElementById('notification-center-unread-only')?.addEventListener('change', event => {
        forumV2State.notificationCenterUnreadOnly = event.target.checked;
        renderNotificationCenter();
    });
    document.getElementById('notification-pref-forum')?.addEventListener('change', async event => {
        try { await saveNotificationCenterPreference('forum_enabled', event.target.checked); }
        catch (error) { event.target.checked = !event.target.checked; v2Toast(error.message || 'Modification impossible.'); }
    });
    document.getElementById('notification-pref-private')?.addEventListener('change', async event => {
        try { await saveNotificationCenterPreference('private_messages_enabled', event.target.checked); }
        catch (error) { event.target.checked = !event.target.checked; v2Toast(error.message || 'Modification impossible.'); }
    });
    document.getElementById('notification-sub-studies')?.addEventListener('change', async event => {
        try { await window.setInfoSubscriptionPreference?.('studies', event.target.checked); await loadNotificationCenterPreferences(); }
        catch (error) { event.target.checked = !event.target.checked; v2Toast(error.message || 'Modification impossible.'); }
    });
    document.getElementById('notification-sub-projects')?.addEventListener('change', async event => {
        try { await window.setInfoSubscriptionPreference?.('projects', event.target.checked); await loadNotificationCenterPreferences(); }
        catch (error) { event.target.checked = !event.target.checked; v2Toast(error.message || 'Modification impossible.'); }
    });
...
async function initForumV2() {
    bindForumV2Events();
    // Attend que la session principale soit résolue avant d’initialiser ce module.
    let attempts = 0;
