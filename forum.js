...
async function saveAdminCategory(event) {
    ...
    const { error } = await forumClient().rpc('admin_save_forum_category', payload);
    if (error) return forumToast(error.message || 'Enregistrement impossible. Exécutez la migration V2.4.');
...
async function deleteAdminCategory(id) {
    ...
    if (!accepted) return;
    const { data, error } = await forumClient().rpc('admin_delete_forum_category', { p_category_id: Number(id) });
    if (error) return forumToast(error.message || 'Suppression impossible. Exécutez la migration V2.4.');
    forumState.categories = [];
    await loadForumAdmin();
    if (forumState.currentView === 'discussions' && !forumState.currentTopicId) await renderForumHome();
...
async function updateAdminUserRole(userId, role) {
    if (!(await ensureForumAdminAccess())) return;
    const { error } = await forumClient().rpc('admin_set_forum_user_role', { p_user_id: userId, p_role: role });
    if (error) forumToast(error.message || 'Modification impossible. Exécutez la migration V2.4.');
...
async function toggleAdminUserBan(button) {
    ...
    const { error } = await forumClient().rpc('admin_set_user_suspended', { p_user_id: userId, p_suspended: !banned });
    button.disabled = false;
    if (error) forumToast(error.message || 'Modification impossible. Vérifiez que la migration V2.4 a bien été exécutée.');
...
async function deleteAdminUser(button) {
    ...
    if (error) {
        button.disabled = false;
        return forumToast(error.message || 'Suppression impossible. Vérifiez que la migration V2.4 a bien été exécutée.');
    }
