...
async function checkForumAdminBackend() {
    ...
    if (!client || !forumState.user) return false;
    const { data, error } = await client.rpc('admin_forum_healthcheck');
    if (error) {
        if (isMissingForumRpc(error)) {
            forumAdminStatus('Les fonctions d’administration Supabase ne sont pas installées ou le cache API n’est pas à jour. Exécutez forum_admin_repair_v2_6.sql dans le SQL Editor.', 'error');
        } else {
            forumAdminStatus(`Vérification Supabase impossible : ${error.message}`, 'error');
        }
        return false;
    }
    if (!data?.is_admin) {
        forumAdminStatus('Votre session est connectée mais Supabase ne reconnaît pas ce compte comme administrateur du site.', 'error');
        return false;
    }
    forumAdminStatus('Administration Supabase opérationnelle.', 'success');
    return true;
}
...
async function ensureForumAdminAccess() {
    ...
    try {
        ...
        if (error || data !== true) {
            forumToast(error?.message || 'Accès administrateur requis.');
            return false;
        }
        ...
        forumToast('Impossible de vérifier la session administrateur.');
        return false;
    }
}
...
async function saveAdminCategory(event) {
    ...
    const payload = {
        p_id: id || null,
        p_slug: document.getElementById('admin-forum-category-slug').value.trim().toLowerCase(),
        p_name: document.getElementById('admin-forum-category-name').value.trim(),
        p_icon: document.getElementById('admin-forum-category-icon').value.trim() || 'fa-solid fa-comments',
        p_sort_order: Number(document.getElementById('admin-forum-category-order').value || 0),
        p_is_active: document.getElementById('admin-forum-category-active').checked,
        p_description: document.getElementById('admin-forum-category-description').value.trim()
    };
    if (!payload.p_slug || !payload.p_name) return forumToast('Le nom et l’identifiant de la catégorie sont obligatoires.');

    const client = forumClient();
    let { error } = await client.rpc('admin_save_forum_category', payload);

    // Secours : les catégories peuvent être gérées directement grâce au RLS admin
    // même si l'ancienne migration RPC a été oubliée.
    if (error && isMissingForumRpc(error)) {
        const row = {
            slug: payload.p_slug,
            name: payload.p_name,
            icon: payload.p_icon,
        ...
        const fallback = id
            ? await client.from('forum_categories').update(row).eq('id', id).select('id').maybeSingle()
            : await client.from('forum_categories').insert(row).select('id').maybeSingle();
...
async function deleteAdminCategory(id) {
    ...
    const client = forumClient();
    let { data, error } = await client.rpc('admin_delete_forum_category', { p_category_id: Number(id) });

    if (error && isMissingForumRpc(error)) {
        // Fallback RLS : permet de réparer immédiatement la gestion des catégories
        // même si la migration RPC V2.4 n'a pas été exécutée.
        const { count, error: countError } = await client
            .from('forum_topics')
            .select('id', { count: 'exact', head: true })
            .eq('category_id', Number(id));
        if (countError) error = countError;
        else {
            let targetCategoryId = null;
            if ((count || 0) > 0) {
                const { data: alternatives, error: altError } = await client
                    ...
                    .order('sort_order')
                    .limit(20);
                if (altError) error = altError;
                else {
                    const preferred = (alternatives || []).sort((a, b) => (a.slug === 'general' ? -1 : b.slug === 'general' ? 1 : 0))[0];
                    if (!preferred) {
                        error = { message: 'Créez une autre catégorie avant de supprimer celle-ci : elle contient encore des discussions.' };
                    } else {
                        targetCategoryId = Number(preferred.id);
                        const moved = await client.from('forum_topics').update({ category_id: targetCategoryId, updated_at: new Date().toISOString() }).eq('category_id', Number(id));
                        if (moved.error) error = moved.error;
                    }
                }
            }
            if (!error) {
                const removed = await client.from('forum_categories').delete().eq('id', Number(id));
                error = removed.error;
                data = { moved_topics: Number(count || 0), target_category_id: targetCategoryId };
            }
        }
    }
...
async function updateAdminUserRole(userId, role) {
    if (!(await ensureForumAdminAccess())) return;
    const client = forumClient();
    let { error } = await client.rpc('admin_set_forum_user_role', { p_user_id: userId, p_role: role });
    if (error && isMissingForumRpc(error)) {
        const fallback = await client.from('profiles').update({ role, updated_at: new Date().toISOString() }).eq('user_id', userId).select('user_id,role').maybeSingle();
        error = fallback.error || (fallback.data?.role !== role ? { message: 'Supabase a refusé le changement de rôle.' } : null);
    }
    if (error) {
...
async function toggleAdminUserBan(button) {
    ...
    if (!banned) {
        const accepted = await siteConfirm({
            ...
            message: 'Le compte pourra toujours consulter le site, mais ne pourra plus publier, réagir ou envoyer de nouveaux messages privés.',
            detail: 'Vous pourrez le réactiver à tout moment depuis cette page.',
    ...
    const client = forumClient();
    let { error } = await client.rpc('admin_set_user_suspended', { p_user_id: userId, p_suspended: !banned });
    if (error && isMissingForumRpc(error)) {
        const wanted = !banned;
        const fallback = await client.from('profiles')
            .update({ is_banned: wanted, updated_at: new Date().toISOString() })
            .eq('user_id', userId)
            .select('user_id,is_banned')
            .maybeSingle();
        error = fallback.error || (fallback.data?.is_banned !== wanted ? { message: 'Supabase n’a pas appliqué la suspension. Vérifiez les droits administrateur/RLS.' } : null);
    }
...
async function deleteAdminUser(button) {
    ...
    const accepted = await siteConfirm({
        ...
        message: 'Le compte utilisateur sera supprimé définitivement.',
        detail: 'Ses sujets et messages publics seront conservés sous « Utilisateur supprimé ». Ses données personnelles, messages privés, réactions, abonnements et statistiques personnelles seront supprimés. Cette action est irréversible.',
    ...
    if (avatarPath) {
        const { error: avatarError } = await forumClient().storage.from('avatars').remove([avatarPath]);
        if (avatarError) console.warn('Nettoyage avatar :', avatarError);
    }
    const { error } = await forumClient().rpc('admin_delete_forum_user', { p_user_id: userId });
    if (error) {
        button.disabled = false;
        console.error('Suppression utilisateur :', error);
        const message = isMissingForumRpc(error)
            ? 'La fonction de suppression administrateur manque côté Supabase. Exécutez forum_admin_repair_v2_6.sql puis rechargez la page.'
            : (error.message || 'Suppression impossible.');
        forumAdminStatus(message, 'error');
        return forumToast(message);
    }
...
function bindForumEvents() {
    ...
    document.addEventListener('click', async event => {
        ...
        if (deleteCat) { await deleteAdminCategory(deleteCat.dataset.adminDeleteCategory); return; }
        ...
        if (ban) { await toggleAdminUserBan(ban); return; }
        const deleteUser = target.closest('[data-admin-user-delete]');
        if (deleteUser) { await deleteAdminUser(deleteUser); return; }
    });
    ...
    document.addEventListener('click', event => {
        const external = event.target.closest?.('#pdf-external-link, #odt-download-link');
