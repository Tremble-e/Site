...
async function deleteAdminCategory(id) {
    if (!forumState.isAdmin || !(await siteConfirm({ title: 'Supprimer cette catégorie ?', message: 'La suppression sera refusée si elle contient encore des sujets.', confirmLabel: 'Supprimer', danger: true }))) return;
...
async function toggleAdminUserBan(button) {
    ...
    if (error) forumToast(error.message || 'Modification impossible. Vérifiez que la migration V2.3 a bien été exécutée.');
...
async function deleteAdminUser(button) {
    ...
    if (error) {
        button.disabled = false;
        return forumToast(error.message || 'Suppression impossible. Vérifiez que la migration V2.3 a bien été exécutée.');
    }
