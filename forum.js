...
async function refreshCurrentAccount(session = null) {
    ...
    window.refreshInfoSubscriptionButtons?.().catch?.(console.warn);
    window.onSiteAccountChanged?.(forumState.user);
}
...
function updateAccountNavigation() {
    ...
    if (favorites) favorites.hidden = false;
...
function renderAccountPopover() {
    ...
    pop.innerHTML = `
        ...
        ${forumState.isAdmin
            ? '<div class="account-menu-divider"></div><button type="button" class="account-menu-action" data-account-action="admin"><i class="fa-solid fa-shield-halved"></i> Administration</button>'
            : forumState.isModerator
                ? '<div class="account-menu-divider"></div><button type="button" class="account-menu-action" data-account-action="info-admin"><i class="fa-solid fa-bullhorn"></i> Gérer les informations</button>'
                : ''}
...
async function logoutForumAccount() {
    ...
    updateAccountNavigation();
    window.refreshInfoSubscriptionButtons?.().catch?.(console.warn);
    window.onSiteAccountChanged?.(null);
...
async function setupNotificationRealtime() {
    ...
    forumState.notificationChannel = client.channel(`notifications-${forumState.user.id}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${forumState.user.id}` }, () => { loadNotifications(); window.refreshNotificationCenter?.(); })
        .subscribe();
}
...
async function renderTopicList({ authorId = forumState.currentAuthorFilter } = {}) {
    ...
    `;
    window.refreshFavoriteButtons?.(container);
}

function forumPresenceMarkup(userId, username = '', withText = false) {
    if (!userId) return '';
    const online = typeof window.isForumUserOnlineV2 === 'function'
        ? window.isForumUserOnlineV2(userId, username)
        : false;
    return `<span class="forum-user-presence ${online ? 'is-online' : ''} ${withText ? 'with-text' : ''}" data-forum-presence-user="${fAttr(userId)}" data-forum-presence-name="${fAttr(username)}" ${withText ? 'data-forum-presence-text="true"' : ''} aria-label="${online ? 'En ligne' : 'Hors ligne'}"><span class="forum-user-presence-dot" aria-hidden="true"></span>${withText ? `<span class="forum-user-presence-label">${online ? 'En ligne' : 'Hors ligne'}</span>` : ''}</span>`;
}
    ...
    return `
        <article class="topic-card" data-forum-topic="${fAttr(topic.id)}" tabindex="0" role="button">
            ...
            <div class="topic-side">
                ${window.siteFavoriteButtonMarkup?.('topic', topic.id, 'Ajouter cette discussion aux favoris', 'topic-favorite-btn') || ''}
                <strong>${replyCount} réponse${replyCount === 1 ? '' : 's'}</strong><span>activité ${fRelativeDate(topic.last_post_at)}</span>
            </div>
        </article>
    `;
}
...
function renderForumTopic() {
    ...
    const headerActions = `
        ${window.siteFavoriteButtonMarkup?.('topic', topic.id, 'Ajouter cette discussion aux favoris', 'forum-mini-btn topic-header-favorite') || ''}
...
function bindForumEvents() {
    ...
    // Les éléments du forum sont créés dynamiquement.
    document.addEventListener('click', async event => {
        ...
        if (accountAction) {
            ...
            if (action === 'info-admin') { closeAccountPopover(); window.openAdminModal?.(); }
        ...
        if (target.closest('[data-favorite-type]')) return;
...
async function initForumModule() {
    ...
    client.auth.onAuthStateChange((event, newSession) => {
        // Décale le chargement pour sortir du callback GoTrue.
        setTimeout(async () => {
