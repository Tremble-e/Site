        ...
        <article class="topic-card" data-forum-topic="${fAttr(topic.id)}" tabindex="0" role="button">
            <div>
                ...
                <div class="topic-meta">
                    <span>par <button type="button" data-member-profile="${fAttr(topic.author_id)}">${fEsc(topic.author_username || 'Membre')}</button></span>
        ...
        <article class="forum-topic-header glass-panel">
            <div class="forum-topic-header-top">
                <div>
                    ...
                    <div class="topic-meta"><span>créé par <button type="button" data-member-profile="${fAttr(topic.author_id)}">${fEsc(topic.author_username || 'Membre')}</button></span><span>•</span><span>${fFullDate(topic.created_at)}</span></div>
                </div>
...
function renderForumPost(post, number) {
    const profile = forumState.profilesById.get(post.author_id) || { user_id: post.author_id, username: 'Membre', role: 'member', created_at: post.created_at };
    const reactions = forumState.reactionsByPost.get(Number(post.id)) || [];
    const mine = forumState.user?.id === post.author_id;
    ...
    const canReport = !post.deleted_at && forumState.user && !mine;
...
async function handleReplySubmit(event) {
    ...
    const { error } = await forumClient().from('forum_posts').insert({ topic_id: forumState.currentTopicId, author_id: forumState.user.id, content });
    if (error) {
...
async function deleteForumPost(postId) {
    if (!confirm('Supprimer ce message ? Il restera indiqué comme supprimé dans la discussion.')) return;
...
async function saveAdminCategory(event) {
    ...
    const query = id ? forumClient().from('forum_categories').update(payload).eq('id', id) : forumClient().from('forum_categories').insert(payload);
    const { error } = await query;
    if (error) return forumToast(error.message || 'Enregistrement impossible.');
...
async function deleteAdminCategory(id) {
    if (!forumState.isAdmin || !confirm('Supprimer cette catégorie ? Cette opération est refusée si elle contient encore des sujets.')) return;
    const { error } = await forumClient().from('forum_categories').delete().eq('id', Number(id));
    if (error) return forumToast(error.message?.includes('foreign key') ? 'Cette catégorie contient encore des sujets.' : (error.message || 'Suppression impossible.'));
    forumState.categories = [];
    await loadForumAdmin();
}
...
async function loadAdminUsers() {
    ...
    el.innerHTML = users.length ? users.map(p => {
        ...
        return `<div class="admin-list-item admin-user-row"><div class="admin-item-main">${fAvatar(p)}<div><strong>${fEsc(p.username)}</strong><small>Inscrit ${fRelativeDate(p.created_at)}${p.is_banned ? ' · SUSPENDU' : ''}</small></div></div><span class="admin-user-activity">${Number(s.message_count || 0)} msg · ${Number(s.topic_count || 0)} sujets · ${Number(s.document_count || 0)} docs</span><select data-admin-user-role="${fAttr(p.user_id)}"><option value="member" ${p.role === 'member' ? 'selected' : ''}>Membre</option><option value="moderator" ${p.role === 'moderator' ? 'selected' : ''}>Modérateur</option><option value="admin" ${p.role === 'admin' ? 'selected' : ''}>Admin</option></select><button type="button" class="${p.is_banned ? 'primary-btn' : 'secondary-btn'} compact-btn" data-admin-user-ban="${fAttr(p.user_id)}" data-banned="${p.is_banned ? '1' : '0'}" ${self ? 'disabled data-tooltip="Vous ne pouvez pas suspendre votre propre compte"' : ''}>${p.is_banned ? 'Réactiver' : 'Suspendre'}</button></div>`;
    }).join('') : '<div class="admin-empty">Aucun utilisateur.</div>';
}

async function updateAdminUserRole(userId, role) {
    if (!forumState.isAdmin) return;
    const { error } = await forumClient().from('profiles').update({ role }).eq('user_id', userId);
    if (error) forumToast(error.message || 'Modification impossible.');
    else { forumToast('Rôle mis à jour.'); await loadAdminUsers(); }
}
...
async function toggleAdminUserBan(button) {
    ...
    if (!banned && !confirm('Suspendre ce compte ? Il pourra toujours consulter le site mais ne pourra plus publier.')) return;
    const { error } = await forumClient().from('profiles').update({ is_banned: !banned }).eq('user_id', userId);
    if (error) forumToast(error.message || 'Modification impossible.');
    else { forumToast(banned ? 'Compte réactivé.' : 'Compte suspendu.'); await loadAdminUsers(); }
}
...
function bindForumEvents() {
    ...
    document.addEventListener('click', async event => {
        ...
        if (editCat) {
            const list = document.getElementById('admin-forum-category-list');
            const cats = JSON.parse(list?.dataset.categories || '[]');
            openAdminCategoryEditor(cats.find(c => Number(c.id) === Number(editCat.dataset.adminEditCategory)) || null);
            return;
        }
