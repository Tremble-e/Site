(() => {
    ...
    const EXTENSION_PACKAGE_URL = './downloads/planilim-collector-v4.16.2.zip';
    ...
    function launchExtensionInstall() {
        ...
        link.href = url;
        link.download = 'planilim-collector-v4.16.2.zip';
        link.rel = 'noopener';
    ...
    async function runCollectorPipeline(profile = COLLECTOR_PROFILES.program, progressContext = {}, targetOverride = null) {
        if (!state.isAdmin || state.busy) return { ok: false, code: 'BUSY' };
        const status = byId('planning-collector-status');
        ...
        const queueTargetsForRecovery = targets => {
            for (const target of Array.isArray(targets) ? targets : []) {
                if (target?.resourceId == null) continue;
                rememberFailure({
                    target,
                    code: target.code || 'SYNC_NOT_PROCESSED',
                    message: target.message || 'EDT prévu dans la synchronisation mais pas encore traité.'
                });
            }
        };

        const persistCurrentRecoveryQueue = async () => {
        ...
        queueTargetsForRecovery(targetOverride);
        ...
        try {
            if (Array.isArray(targetOverride) && targetOverride.length) {
                await persistCurrentRecoveryQueue();
            }
            ...
            const started = await requestExtension('PLANILIM_COLLECTOR_ASYNC_START', {
                ...
                payload: {
                    ...
                    targets: Array.isArray(targetOverride) ? targetOverride : []
                }
            });
            ...
            while (guard++ < 7200) {
                ...
                if (discoveredTargets.length) {
                    ...
                    queueTargetsForRecovery(discoveredTargets);
                    await persistCurrentRecoveryQueue();
                    renderCollectorRows();
                }
    ...
    async function publishCollectorPayloads(resourceIds = null) {
        ...
        if (requested != null) {
            const publishableIds = new Set(rows.map(item => Number(item.resource_id)));
            const missingIds = [...requested].filter(resourceId => !publishableIds.has(Number(resourceId)));
            if (missingIds.length) {
                return {
                    ok: false,
                    code: 'COLLECTOR_PAYLOAD_SET_INCOMPLETE',
                    missingResourceIds: missingIds
                };
            }
        }
        ...
        return {
            ok: true,
            code: 'COLLECTOR_PUBLISHED',
            count: rows.length,
            resourceIds: rows.map(item => Number(item.resource_id))
        };
    }
    ...
    async function retryFailedCollectorResources(kind = 'all') {
        ...
        let remaining = failureKinds.flatMap(current => collectorFailuresForKind(current));
        ...
        try {
            const results = [];
            for (let profileIndex = 0; profileIndex < failureKinds.length; profileIndex += 1) {
                const profile = collectorProfile(failureKinds[profileIndex]);
                const targets = collectorFailuresForKind(profile.key);
                if (!targets.length) continue;
                if (status) {
                    status.textContent = `Rattrapage ${profile.label.toLowerCase()} : ${targets.length} emploi${targets.length > 1 ? 's' : ''} du temps · 4 workers…`;
                }
                const result = await runCollectorPipeline(profile, {
                    phaseIndex: profileIndex,
                    phaseCount: failureKinds.length
                }, targets);
                results.push(result || { ok: false, profile: profile.key });
                if (result?.authRequired || result?.code === 'AUTH_REQUIRED') break;
            }

            remaining = failureKinds.flatMap(current => collectorFailuresForKind(current));
